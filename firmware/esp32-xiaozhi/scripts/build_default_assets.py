...
def remove_tree_with_retry(path, attempts=8):
    """Best-effort cleanup for transient Windows file-handle locks."""
    for attempt in range(attempts):
        try:
            ...
            if attempt == attempts - 1:
                print(f"Warning: Could not clean temporary directory {path}: {error}")
                return False
...
def build_assets_integrated(wakenet_model_paths, multinet_model_paths, text_font_path, emoji_collection_path, extra_files_path, output_path, multinet_model_info=None):
    ...
    try:
        ...
        if os.path.exists(temp_build_dir):
            if not remove_tree_with_retry(temp_build_dir):
                return False
        ...
        if os.path.exists(temp_build_dir):
            remove_tree_with_retry(temp_build_dir)
