...
void McpServer::GetToolsList(int id, const std::string& cursor, bool list_user_only_tools) {
    ...
    while (it != tools_.end()) {
        if (std::count_if(tools_.begin(), it, [list_user_only_tools](const McpTool* t) {
                return list_user_only_tools || !t->user_only();
            }) >= static_cast<int>(kMaxAdvertisedTools)) {
            break;
        }
        // 如果我们还没有找到起始位置，继续搜索
        if (!found_cursor) {
    ...
    // Never expose a cursor for the count limit: the peer would request the
    // next page and hit its hard 32-tool limit again.
    if (next_cursor.empty() || std::count_if(tools_.begin(), it, [list_user_only_tools](const McpTool* t) {
            return list_user_only_tools || !t->user_only();
        }) >= static_cast<int>(kMaxAdvertisedTools)) {
        next_cursor.clear();
    }
