...
private:
    ...
    std::recursive_timed_mutex camera_mutex_;

    bool CaptureLocked();
    bool AnalyzeCurrentFrame(CameraNavigationMetrics& metrics) const;
...
public:
    ...
    virtual bool AnalyzeNavigation(CameraNavigationMetrics& metrics) override;
    bool TryAcquireVision(uint32_t timeout_ms) override;
    void ReleaseVision() override;
    virtual std::string Explain(const std::string &question) override;
    bool DumpJpegToConsole();
};
